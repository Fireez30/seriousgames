...
function init()
	...
	solids={[34]=true}
	ent={
	[0]={x=50,y=30,a=0,speed=0.1,sprite=50,health=50,dead=0}
	}
end
flip=0
function solid(x,y)
...
function TIC()
	...
	local opp = null
	if mget((p.x+cam.x)//8,(p.y+cam.y)//8) > 48 then 
		local opp = mget((p.x+cam.x)//8,(p.y+cam.y)//8)
	elseif mget((p.x+7+cam.x)//8,(p.y+cam.y)//8) > 48 then
		opp = mget((p.x+7+cam.x)//8,(p.y+cam.y)//8)
	elseif mget((p.x+cam.x)//8,(p.y+7+cam.y)//8) > 48 then
		opp = mget((p.x+cam.x),(p.y+7+cam.y)//8)
	elseif mget((p.x+7+cam.x)//8,(p.y+7+cam.y)//8)> 48 then
		opp = mget((p.x+7+cam.x)//8,(p.y+7+cam.y)//8)
	end
